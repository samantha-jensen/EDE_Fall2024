...
df <- data.frame(c("Filename", "Data", "Source"), c(Filename, Date, Source))
# making table from dataframe 
knitr::kable(df, col.names = c("Item", "Value"), align = c("r", "l"))
```
...
# table for Mean Particulates
kable(summary_PM25, col.names = c("County", "µg/m3"), digits = 2, 
      caption = "Mean Particulates (2.5mm)")
# Table for Ozone 
kable(summary_Ozone, col.names = c("County", "µg/m3"), digits = 2, 
      caption = "Mean Ozone")
```
...
```{r echo= FALSE, message=FALSE, warning=FALSE, fig.align= 'left', fig.cap= 'Ozone Levels in 2018', fig.height = 3}
epa_data$Month <- factor(epa_data$Month, level = 1:12, labels = month.abb)
plot_2018 <- epa_data %>% 
  drop_na(Ozone) %>% 
  filter(Year == "2018") %>% 
...
```{r echo = FALSE, message=FALSE, warning=FALSE, fig.align= 'left', fig.cap= 'Ozone Levels in 2019', fig.height = 3}
plot_2019 <- epa_data %>% 
  drop_na(Ozone) %>% 
  filter(Year == "2019") %>% 
  ggplot(aes(x = Month, y = Ozone, group = Month)) +
...
## Git Repository
https://github.com/samantha-jensen/EDE_Fall2024
