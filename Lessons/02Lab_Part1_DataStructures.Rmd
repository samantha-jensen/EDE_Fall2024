...
# Combine vectors with the same data type 
c(vector1, vector4)

# Multiplies the corresponding elements in the vectors together (first * first etc.)
vector1 * vector4

# Multiplies each element in the vector by 10 
vector1 * 10

# Changes lower case into upper case 
toupper(vector2) 

# Denying the object (true becomes false etc.)
!vector3
...
matrix3 <- matrix(
  ...
  nrow = 5, 
  ncol = 4, 
  byrow = TRUE, # return after comma continues the line // dimnames = how to name columns and rows 
  dimnames = list(
...
matrix1[c(12, 14)] # specify elements to call 

matrix1[c(12:14)] # range to call 

matrix1[2:4, 1:3] # small matrix within matrix1
...
# $ calls the name of the object in the list 
list1$age

# 
names(list1)

# Return the first 2 elements of the list 
list1[1:2]

# change the value of an object in the list 
list1$name = "Janelle"

# get the value of the 4th element (need to use double brackets when using the values in functions, can't refer to the object)
list1[[4]] = 44
```
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # make sure you comment out "view" before you knit 
