...
huc8_sf_filter <- st_read(here('Data/Spatial/NCHUC8.shp')) %>% 
  filter(SUBBASIN == "Upper Neuse")

#Create a map
mapview(huc8_sf, col.regions = 'grey') +
  mapview(huc8_sf_filter, col.region = 'yellow', color = 'blue')

ggplot(data=huc8_sf) +
  geom_sf() +
  geom_sf(data = huc8_sf_filter, fill = 'yellow', color = 'blue')
...
wind_data <-  wind_data %>% 
  mutate(HUC_8 = substr(wind_data$HUC_12, 1, 8)) 
wind_data <- wind_data %>% 
  group_by(HUC_8) %>% 
  summarise(AvgWindEnergy = sum(AvgWindEnergy))
 #Join to HUC_8 features
 joined_huc8 <- left_join(huc8_sf, wind_data, by = "HUC_8") %>% 
   drop_na(FID)

#View the outputs
 mapview(joined_huc8, zcol = 'AvgWindEnergy')
...
# How do you get it to keep the geometry values?
huc2_sf <- agr %>% summarise(ACRES = sum(ACRES))
...
tri_intersect <- counties_sf_utm %>% 
  st_filter(triCo, .predicate = st_intersects)


#Intersect the HUC_8s
triIntersect <- triCo %>% st_intersection(counties_sf_utm)

mapview(triIntersect, zcol = 'ALAND') 
...
site_buffer <- st_buffer(theSite_sfc_transformed, dist = 30000)
close_counties <-  counties_sf_utm %>% st_intersection(site_buffer)

mapview(close_counties, zcol = 'NAME') + mapview(theSite_sfc_transformed)
