...
#create a new vector from first to last date sequence before inner join to check for NAs
...
BR_complete_rain_ts <- ts(BR_complete$Rainfall, start = c(1901,1), 
                          frequency = 12)
BR_complete_temp_ts <- ts(BR_complete$Temperature_C, start = c(1901,1), 
                          frequency = 12)
...
BR_rain_decomposed <- stl(BR_complete_rain_ts, s.window = "periodic")
plot(BR_rain_decomposed)
#bars on the right indicate how important each compoenent is. smaller bar = stronger component
# similar magnitudes = both playing an important role in varability 
...
BR_temp_componenets <- mutate(BR_temp_components,
        Observed = Temp_BR_processed$Temperature_C,     
        Date = Temp_BR_processed$Date)
ggplot(BR_temp_componenets) +
...
SeasonalMannKendall(BR_complete_rain_ts)

SeasonalMannKendall(BR_complete_temp_ts)





s# Exercise 12: Now remove the seasonality and apply another test. Compare the results with what you obtained on Exercise 9.
