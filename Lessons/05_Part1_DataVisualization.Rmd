...
# Fix this code Color inside aesthetic = adding another layer to the plot... to just change the color 
...
NvsP <-
  ...
  geom_point() +
  # lm = linear model geom_smooth() adding a trend line 
  geom_smooth(method = lm) +
 # if you know the slope and the intercept of the line 
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? Fill only fills the box while keeping the contour black.. doesn't look good 
print(Nutrientplot3)
...
# Violin plot - shows density plot as well as frequency 
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
# Ridgeline plot - density of distribution over time 
Nutrientplot6 <-
