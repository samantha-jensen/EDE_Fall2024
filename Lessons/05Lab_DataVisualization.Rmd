...
month_obs <- ggplot(PeterPaul.chem.nutrients) + 
...
# Should input theme with the libraries at the beginning 
...
temperature_plot <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0) %>% 
  ggplot(mapping = aes(x = daynum, y=temperature_C)) +
  geom_point()
temperature_plot
...
date_plot <- PeterPaul.chem.nutrients %>% 
