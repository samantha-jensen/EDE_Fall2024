...
gage_data <- read.table(
  theURL,
  skip = 27, # where the code actually starts 
  header=TRUE,
  sep='\t',
  stringsAsFactors = T) 
...
colnames(gage_data24) = c(
  "agency_cd", "site_no", "datetime", "tz_cd",
  "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")

gage_data24 <- gage_data24 %>% 
  select(datetime, discharge_cfs, gage_ht_ft) %>% 
    mutate(datetime = ymd_hm(datetime)) 

ggplot(gage_data24,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
...
df_all <- seq(2020,2023) %>% map(scrape.it) %>% bind_rows()
```