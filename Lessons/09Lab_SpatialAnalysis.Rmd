...
power_plants <- st_read('https://services1.arcgis.com/Hp6G80Pky0om7QvQ/arcgis/rest/services/Plants_gdb/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson')

power_plants_clean <- power_plants %>% 
  filter(STATE == "NC")
st_crs(power_plants_clean)
```

#Now we can make a pretty map!
```{r cluster map}
#Create the map
cluster_map <- leaflet(power_plants_clean) %>% 
  addTiles() %>% 
  addMarkers(
