...
# %in% look through all the rows and keep in if match 
...
NTL.phys.data.tuesday <- filter(NTL.phys.data, lakename %in% c("Tuesday Lake") & 
...
> Answer: row numbers can change.. want to filter in the data itself 
...
NTL.phys.data.tues <- NTL.phys.data %>% filter(lakename == "Tuesday Lake") %>% 
...
NTL.phys.data.water <- filter(NTL.phys.data.tues, depth == 0) %>% 
